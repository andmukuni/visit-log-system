...
import { seedPortalUsers } from '../seedPortalUsers.js';
...
async function resolvePool() {
  ...
  if (connectionString) {
    ...
    console.log('[seed:portal-users] Connecting to PostgreSQL (explicit URL)…');
  ...
  if (isPostgresDriver(resolveDbDriver())) {
    ...
    console.log('[seed:portal-users] Connecting to local PostgreSQL…');
    return createPgPool(localPgUrl);
  }

  console.log('[seed:portal-users] Connecting to local MySQL…');
...
main().catch((error) => {
  console.error('[seed:portal-users] Failed:', error.message);
  process.exit(1);
});
