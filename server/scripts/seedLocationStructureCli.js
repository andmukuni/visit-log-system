...
import { resetAndSeedLocationStructure } from '../visitorSchema.js';
...
async function resolvePool() {
  ...
  if (connectionString) {
    ...
    console.log('[seed:location] Connecting to PostgreSQL (explicit URL)…');
  ...
  if (isPostgresDriver(resolveDbDriver())) {
    ...
    console.log('[seed:location] Connecting to local PostgreSQL…');
    return createPgPool(localPgUrl);
  }

  console.log('[seed:location] Connecting to local MySQL…');
...
async function main() {
  ...
  try {
    ...
    console.log('[seed:location] Done.', {
...
main().catch((error) => {
  console.error('[seed:location] Failed:', error.message);
  process.exit(1);
});
