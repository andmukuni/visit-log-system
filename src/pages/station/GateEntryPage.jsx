...
import GateCheckOutPanel from './GateCheckOutPanel';
...
function CheckInSignatureStep({ subjectName, signatureLabel, signature, onSignatureChange }) {
  return (
    <FormSection
    ...
    >
      <div className="mb-4 flex items-center gap-3 rounded-xl border border-cyan-100 bg-cyan-50/60 px-4 py-3">
        ...
        <p className="text-sm text-navy-700">
          This signature confirms check-in at the gate.
        </p>
      </div>
      <SignaturePad
        value={signature}
...
function StepActions({ step, totalSteps, onBack, onNext, onFinish, submitting }) {
...
export default function GateEntryPage() {
  ...
  const [section, setSection] = useState(() => resolveSection(searchParams.get('tab')));
  const [mode, setMode] = useState(() => (
    resolveSection(searchParams.get('tab')) === 'checkout' ? 'walk-in' : 'vehicle'
  ));
  ...
  const loadRef = useCallback(async () => {
    try {
      setRefData(await visitorApi.getReferenceData());
    } catch {
  ...
  const verifyNrc = async () => {
    ...
    setDojahOverride(false);
    try {
      const result = await visitorApi.gateEntryNrcLookup(nrc);
      setNrcLookup({
        status: 'verified',
  ...
  const handlePlateBlur = async () => {
    ...
    try {
      const result = await visitorApi.searchVehicleByPlate(plate);
  ...
  const submitWalkIn = async (e) => {
    ...
    setSubmitting(true);
    try {
      const visit = await visitorApi.gateEntryWalkIn({
        ...walkIn,
        phone: buildFullPhone(walkIn.phoneCountry, walkIn.phone),
        dojahOverride: dojahOverride || undefined,
        checkInSignature,
      });
      toast.success(`Entry logged. Pass code: ${visit.pass_code || '—'}`);
      resetForm();
    } catch (err) {
      if (err.unavailable) {
        setNrcLookup({
  ...
  const submitVehicle = async (e) => {
    ...
    setSubmitting(true);
    try {
      const result = await visitorApi.gateEntryVehicle({
        ...vehicle,
        phone: buildFullPhone(vehicle.phoneCountry, vehicle.phone),
        checkInSignature,
      });
      toast.success(`Vehicle ${result.vehicle?.plate_number} allowed entry.`);
      resetForm();
    } catch (err) {
      toast.error(err.message);
    } finally {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-navy-50">
      <PageLogoWatermark />
