export const VEHICLE_STATUS_OPTIONS = [
  { value: '', label: 'All', dot: 'bg-navy-300' },
  { value: 'on_site', label: 'On site', dot: 'bg-emerald-400' },
  { value: 'checked_out', label: 'Checked out', dot: 'bg-navy-400' },
  { value: 'expected', label: 'Expected', dot: 'bg-amber-400' },
];

export const VISIT_LOG_STATUS_OPTIONS = [
  { value: '', label: 'All', dot: 'bg-navy-300' },
  { value: 'pending_approval', label: 'Pending approval', dot: 'bg-amber-400' },
  { value: 'approved', label: 'Approved', dot: 'bg-sky-400' },
  { value: 'checked_in', label: 'Checked in', dot: 'bg-emerald-400' },
  { value: 'completed', label: 'Completed', dot: 'bg-navy-400' },
  { value: 'rejected', label: 'Rejected', dot: 'bg-rose-400' },
];
